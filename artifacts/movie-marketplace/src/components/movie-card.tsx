...
export const QualityBadge = ({ quality, className }: { quality: string, className?: string }) => {
  const colors: Record<string, string> = {
    '720p': 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20',
    '1080p': 'bg-blue-500/10 text-blue-400 border-blue-500/20',
    '4K': 'bg-amber-500/10 text-amber-400 border-amber-500/20'
  };
...
export function MovieCard({ movie, index = 0 }: { movie: Movie, index?: number }) {
  return (
    ...
    >
      <Link href={`/movie/${movie.id}`} className="block relative aspect-[2/3] rounded-xl overflow-hidden bg-card border border-white/5">
        <img 
          src={movie.posterUrl} 
          alt={movie.title} 
        ...
        <div className="absolute inset-x-0 bottom-0 p-4 flex flex-col justify-end translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-white/60 text-xs font-mono">{movie.year}</span>
          </div>
          
          <h3 className="font-bold text-white leading-tight mb-1 line-clamp-1">{movie.title}</h3>
          
          <div className="flex items-center justify-between mt-2">
            <span className="text-xs text-white/50 truncate pr-2">{movie.genre.slice(0, 2).join(', ')}</span>
